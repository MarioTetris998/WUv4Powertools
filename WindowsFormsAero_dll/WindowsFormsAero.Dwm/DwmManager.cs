...
public static class DwmManager
{
	...
	/// using <see cref="M:WindowsFormsAero.Dwm.Thumbnail.Update(System.Drawing.Rectangle,System.Drawing.Rectangle,System.Byte,System.Boolean,System.Boolean)" />.
	/// </remarks>
	...
	public static Thumbnail Register(IntPtr destination, IntPtr source)
	{
		...
		if (DwmMethods.DwmRegisterThumbnail(destination, source, out var ret) == 0)
		{
			return new Thumbnail(ret);
		}
		throw new DwmCompositionException(string.Format(ExceptionMessages.NativeCallFailure, "DwmRegisterThumbnail"));
	}
	...
	private static void InternalGlassFrame(IntPtr hWnd, Padding margins)
	{
		if (OsSupport.IsVistaOrLater && OsSupport.IsCompositionEnabled)
		{
			Margins nativeMargins = Margins.FromPadding(margins);
			if (DwmMethods.DwmExtendFrameIntoClientArea(hWnd, ref nativeMargins) != 0)
			{
				throw new DwmCompositionException(string.Format(ExceptionMessages.NativeCallFailure, "DwmExtendFrameIntoClientArea"));
			}
		}
	}
	...
	{
		if (!OsSupport.IsVistaOrLater || OsSupport.IsEightOrLater || !OsSupport.IsCompositionEnabled || DwmMethods.DwmSetWindowFlip3dPolicy(hwnd, policy) == 0)
		{
			return;
		}
	...
	{
		if (!OsSupport.IsSevenOrLater || !OsSupport.IsCompositionEnabled || DwmMethods.DwmSetWindowDisallowPeek(hwnd, disallowPeek) == 0)
		{
			return;
		}
	...
	{
		if (!OsSupport.IsSevenOrLater || !OsSupport.IsCompositionEnabled || DwmMethods.DwmSetWindowExcludedFromPeek(form.Handle, excluded) == 0)
		{
			return;
		}
	...
	/// </summary>
	/// <remarks>Always returns <see cref="F:WindowsFormsAero.Dwm.CloakedStatus.Uncloaked" /> on unsupported OSs.</remarks>
	...
	{
		if (!OsSupport.IsEightOrLater || DwmMethods.DwmSetWindowCloaked(hwnd, cloak) == 0)
		{
			return;
		}
	...
	{
		if (!OsSupport.IsEightOrLater || DwmMethods.DwmSetWindowFreezeRepresentation(hwnd, freeze) == 0)
		{
			return;
		}
