...
internal static class DwmMethods
{
	...
	public static CloakedStatus DwmGetWindowCloaked(IntPtr hwnd)
	{
		DwmGetWindowAttribute(hwnd, DwmWindowAttribute.DWMWA_CLOAKED, out var retValue, Marshal.SizeOf(typeof(int)));
		if (!Enum.IsDefined(typeof(CloakedStatus), retValue))
		{
			return CloakedStatus.Uncloaked;
		}
		return (CloakedStatus)retValue;
	}

	public static bool DwmGetWindowFreezeRepresentation(IntPtr hwnd)
	{
		if (DwmGetWindowAttribute(hwnd, DwmWindowAttribute.DWMWA_CLOAKED, out var retValue, Marshal.SizeOf(typeof(int))) != 0)
		{
			return false;
		}
		return retValue != 0;
