...
{
	private const int defaultProgressBarMax = 100;

	private const int defaultMarqueeSpeed = 50;

	private const CommonButton defaultButton = CommonButton.OK;

	private const CommonIcon defaultIcon = CommonIcon.None;
	...
	private int _ProgressBarMinRange;

	private int _ProgressBarMaxRange = 100;
	...
	/// </summary>
	public bool IsShowing => _hwnd != IntPtr.Zero;
	...
	/// <see cref="T:WindowsFormsAero.TaskDialog.CommonButtonResult" />.
	/// </remarks>
	...
	public string Instruction
	{
		...
		set
		{
			if (IsShowing)
			{
				PostMessage(new Message(NativeMethods.TaskDialogMessages.TDM_SET_ELEMENT_TEXT, Message.DialogElements.TDE_MAIN_INSTRUCTION, value));
			}
			_Instruction = value;
		}
	}
	...
	public string Content
	{
		...
		set
		{
			if (IsShowing)
			{
				PostMessage(new Message(NativeMethods.TaskDialogMessages.TDM_SET_ELEMENT_TEXT, Message.DialogElements.TDE_CONTENT, value));
			}
			_Content = value;
		}
	}
	...
	public string ExpandedInformation
	{
		...
		set
		{
			if (IsShowing)
			{
				PostMessage(new Message(NativeMethods.TaskDialogMessages.TDM_SET_ELEMENT_TEXT, Message.DialogElements.TDE_EXPANDED_INFORMATION, value));
			}
	...
	public string Footer
	{
		...
		set
		{
			if (IsShowing)
			{
				PostMessage(new Message(NativeMethods.TaskDialogMessages.TDM_SET_ELEMENT_TEXT, Message.DialogElements.TDE_FOOTER, value));
			}
			_Footer = value;
		}
	}
	...
	/// <see cref="E:WindowsFormsAero.TaskDialog.TaskDialog.HyperlinkClick" /> event.
	/// </remarks>
	public bool EnableHyperlinks
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_ENABLE_HYPERLINKS);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_ENABLE_HYPERLINKS, value);
		}
	}
	...
	public bool AllowDialogCancellation
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_ALLOW_DIALOG_CANCELLATION);
		}
		set
		{
	...
	public bool UseCommandLinks
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_USE_COMMAND_LINKS);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_USE_COMMAND_LINKS, value);
		}
	}
	...
	public bool UseCommandLinksWithoutIcon
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_USE_COMMAND_LINKS_NO_ICON);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_USE_COMMAND_LINKS_NO_ICON, value);
		}
	}
	...
	public bool ShowExpandedInfoInFooter
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_EXPAND_FOOTER_AREA);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_EXPAND_FOOTER_AREA, value);
		}
	}
	...
	public bool IsExpanded
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_EXPANDED_BY_DEFAULT);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_EXPANDED_BY_DEFAULT, value);
		}
	}
	...
	public bool IsVerificationChecked
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_VERIFICATION_FLAG_CHECKED);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_VERIFICATION_FLAG_CHECKED, value);
		}
	}
	...
	public bool ShowProgressBar
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_SHOW_PROGRESS_BAR);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_SHOW_PROGRESS_BAR, value);
		}
	}
	...
	public bool EnableCallbackTimer
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_CALLBACK_TIMER);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_CALLBACK_TIMER, value);
		}
	}
	...
	public bool PositionRelativeToWindow
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_POSITION_RELATIVE_TO_WINDOW);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_POSITION_RELATIVE_TO_WINDOW, value);
		}
	}
	...
	public bool RightToLeftLayout
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_RTL_LAYOUT);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_RTL_LAYOUT, value);
		}
	}
	...
	public bool NoDefaultRadioButton
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_NO_DEFAULT_RADIO_BUTTON);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_NO_DEFAULT_RADIO_BUTTON, value);
		}
	}
	...
	public bool CanBeMinimized
	{
		get
		{
			return GetConfigFlag(NativeMethods.TaskDialogFlags.TDF_CAN_BE_MINIMIZED);
		}
		set
		{
			SetConfigFlag(NativeMethods.TaskDialogFlags.TDF_CAN_BE_MINIMIZED, value);
		}
	}
	...
	private void Init()
	{
		...
		Content = null;
		CommonIcon = CommonIcon.None;
		CustomIcon = null;
		CommonButtons = (CommonButton)0;
		CustomButtons = null;
		DefaultButton = 1;
		RadioButtons = null;
		...
		Width = 0u;
		config = default(NativeMethods.TaskDialogConfig);
	}
	...
	{
		SetMarqueeProgressBar(enabled, 50);
	}
	...
	internal IntPtr CommonCallbackProc(IntPtr hWnd, uint uEvent, UIntPtr wParam, IntPtr lParam, IntPtr refData)
	{
		...
		case NativeMethods.TaskDialogNotification.TDN_NAVIGATED:
			DispatchMessageQueue();
			this.Navigating?.Invoke(this, EventArgs.Empty);
			break;
		case NativeMethods.TaskDialogNotification.TDN_BUTTON_CLICKED:
		{
			EventHandler<ClickEventArgs> evtButtonClick = this.ButtonClick;
			if (evtButtonClick != null)
			{
				ClickEventArgs args = new ClickEventArgs((int)(uint)wParam);
		...
		case NativeMethods.TaskDialogNotification.TDN_HYPERLINK_CLICKED:
			this.HyperlinkClick?.Invoke(this, new HyperlinkEventArgs(Marshal.PtrToStringUni(lParam)));
			break;
		case NativeMethods.TaskDialogNotification.TDN_TIMER:
		{
			EventHandler<TimerEventArgs> evtTick = this.Tick;
			if (evtTick != null)
			{
				TimerEventArgs args2 = new TimerEventArgs((long)(ulong)wParam);
				evtTick(this, args2);
				return (IntPtr)(args2.ResetCount ? 1 : 0);
			}
			break;
		}
		...
		case NativeMethods.TaskDialogNotification.TDN_RADIO_BUTTON_CLICKED:
			this.RadioButtonClick?.Invoke(this, new ClickEventArgs((int)(uint)wParam));
			break;
		case NativeMethods.TaskDialogNotification.TDN_DIALOG_CONSTRUCTED:
			this.Constructed?.Invoke(this, EventArgs.Empty);
			break;
		case NativeMethods.TaskDialogNotification.TDN_VERIFICATION_CLICKED:
			this.VerificationClick?.Invoke(this, new CheckEventArgs((uint)wParam == 1));
			break;
		case NativeMethods.TaskDialogNotification.TDN_HELP:
			this.Help?.Invoke(this, EventArgs.Empty);
			break;
		case NativeMethods.TaskDialogNotification.TDN_EXPANDO_BUTTON_CLICKED:
			this.Expanding?.Invoke(this, new ExpandEventArgs((uint)wParam != 0));
			break;
		}
		return IntPtr.Zero;
	}
	...
	internal unsafe void PreConfig(IntPtr owner)
	{
		...
		if (CustomButtons != null)
		{
			...
			{
				Marshal.StructureToPtr(CustomButtons[i], (IntPtr)((byte*)(void*)config.pButtons + i * Marshal.SizeOf(typeof(CustomButton))), fDeleteOld: false);
			}
		}
		else
		{
		...
		if (RadioButtons != null)
		{
			...
			{
				Marshal.StructureToPtr(RadioButtons[j], (IntPtr)((byte*)(void*)config.pRadioButtons + j * Marshal.SizeOf(typeof(CustomButton))), fDeleteOld: false);
			}
		}
		else
		{
		...
		config.pfCallback = CommonCallbackProc;
		config.lpCallbackData = IntPtr.Zero;
	}

	/// <summary>Frees the unmanages memory allocated by PreConfig().</summary>
	internal unsafe void PostConfig()
	{
		if (config.pButtons != IntPtr.Zero)
		{
			for (int i = 0; i < config.cButtons; i++)
			{
				Marshal.DestroyStructure((IntPtr)((byte*)(void*)config.pButtons + i * Marshal.SizeOf(typeof(CustomButton))), typeof(CustomButton));
			}
			Marshal.FreeHGlobal(config.pButtons);
		}
		if (config.pRadioButtons != IntPtr.Zero)
		{
			for (int j = 0; j < config.cRadioButtons; j++)
			{
				Marshal.DestroyStructure((IntPtr)((byte*)(void*)config.pRadioButtons + j * Marshal.SizeOf(typeof(CustomButton))), typeof(CustomButton));
			}
	...
	private TaskDialogResult InternalShow(IntPtr owner)
	{
		...
		try
		{
			...
			{
				throw new Exception(string.Format(ExceptionMessages.NativeCallFailure, "TaskDialogIndirect"));
			}
		}
		catch (EntryPointNotFoundException innerException)
		{
			throw new Exception(ExceptionMessages.CommonControlEntryPointNotFound, innerException);
		}
		catch (Exception innerException2)
		{
			throw new Exception(ExceptionMessages.TaskDialogFailure, innerException2);
		}
		finally
		{
			PostConfig();
	...
	private static CommonButtonResult InternalShow(IntPtr parent, string title, string instruction, string content, CommonButton commonButtons, CommonIcon icon)
	{
		...
		try
		{
			...
			{
				parent = Methods.GetActiveWindow();
			}
			...
			{
				throw new Exception(string.Format(ExceptionMessages.NativeCallFailure, "TaskDialog"));
			}
		}
		catch (EntryPointNotFoundException innerException)
		{
